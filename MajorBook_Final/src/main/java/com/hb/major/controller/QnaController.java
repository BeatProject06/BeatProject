...
//		
	...
	@RequestMapping(value = "/question/{postno}", method = RequestMethod.PUT) //수정페이지로 입장
	public String editpagead(Locale locale, Model model, @PathVariable("postno") int no) {

		logger.info("문의사항 관리자 수정 페이지", locale);
		model.addAttribute("currentmenu", "question");
		qnaService.qnaDetailOne(no, model);
	...
	@RequestMapping(value = "/question/completeedit/{postno}", method = RequestMethod.PUT)//수정 실행
	public String editad(@PathVariable("postno") int no, QnaVo bean, Model model) throws Exception {
	...
	@RequestMapping(value = "/question/{postno}", method = RequestMethod.DELETE)
	public String deletead(@PathVariable("postno") int no, Model model) throws Exception {
		System.out.println("delete처음페이지로");
		qnaService.qnaDetailOne(no, model);
		return "user/qna/delete";
	}

	@RequestMapping(value = "/question/delete/{postno}", method = RequestMethod.DELETE)
	public String deletecomplad(@PathVariable("postno") int no, QnaVo bean, Model model, HttpServletRequest req) throws Exception {
	
		String id = req.getParameter("qnaId");
		System.out.println(id);
		qnaService.qnaDeleteOne(no);
		System.out.println(no + "번째 게시글 삭제");
		return "redirect:/question/?userid="+id+"&master=false";//어차피 자기글 삭제되는거 유저니까 권한 false 줌
	}
